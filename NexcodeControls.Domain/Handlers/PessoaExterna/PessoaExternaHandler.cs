...
namespace NexcodeControls.Domain.Handlers
{
        ...
        IHandler<ExcluirPessoaExternaCommand>       
    {
        ...
        public ICommandResult Handle(InserirPessoaExternaCommand command)
        {
            ...
            #region Validações
            ////Verificar se a pessoa externa esta cadastrada
            //NcPessoaExterna pessoaNome = _repository.ObterPorNome(command.Nome);
            //if (pessoaNome != null && pessoaNome.Id > 0)
            //{
            //    return new GenericCommandResult(false, "Pessoa interna está em uso", MensagemTipo.ADVERTENCIA, null);
            //}
            ...
            #region exemplo
            //exemploapenas um exemplo de um serviço do tipo email
            //Enviar email
            //evitar colocar systemNetMail e outras dependencias no dominio
            //_emailService.send(pessoaExterna.Nome.ToString(), pessoaExterna.Email.ToString(), "Bem vindo a Nexcode", "Seu cadastro foi efetuado");
            #endregion

            // Retorna o resultado
            return new GenericCommandResult(true, "Cadastro realizado com sucesso", MensagemTipo.ADVERTENCIA, pessoaExterna);
        }
