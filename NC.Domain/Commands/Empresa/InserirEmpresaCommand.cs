...
namespace NC.Domain.Commands.Entidade
{
    public class InserirEmpresaCommand : Notifiable, ICommand
    {
        #region Contrutores
        public InserirEmpresaCommand() { }

        public InserirEmpresaCommand(string nome, string grupo, string gestor, string telefone1, string telefone2, string email, string cnpj, string ie, string complemento1, string complemento2, byte classificacao, string enderecoLogradouro, string enderecoNumero, string enderecoComplemento, string enderecoBairro, string enderecoCep, string enderecoCidade, string enderecoEstado, string enderecoPais, string observacao, DateTime? dataCadastro)
        {
            Nome = nome;
            Grupo = grupo;
            Gestor = gestor;
        ...
        //[ID(nameof(Site))]
        //public IReadOnlyList<int> SiteIds { get; set; }
